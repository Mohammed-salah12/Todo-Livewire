    ...
    <div id="head" class="flex border-blue-800 border-t-2">
        <div class="w-full">
            <header class="flex bg-white justify-between h-20 border-b border-b-gray-200 items-center px-6">
                <div id="left-header">Leesun Todo runtime</div>
                ...
                <div id="create-form" class="hover:shadow p-6 bg-white border-blue-500 border-t-2">
                    ...
                    <div>
                        <form wire:submit.prevent="createTodo">
                            <div class="mb-6">
                                <label for="title" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">* Todo</label>
                            ...
                            <button type="submit" class="px-4 py-2 bg-blue-500 text-white font-semibold rounded hover:bg-blue-600">Create +</button>
                            @if(session('success'))
                    ...
                    <div class="todo mb-5 card px-5 py-6 bg-white col-span-1 border-t-2 border-blue-500 hover:shadow">
                        <div class="flex justify-between space-x-2">
                            @if($editingTodoId === $todo->id)
                                <input type="text" wire:model="editingTodoName" class="bg-gray-100 text-gray-900 text-sm rounded block w-full p-2.5">
                                @error('editingTodoName')
                                <span class="text-red-500 text-xs block">{{ $message }}</span>
                                @enderror
                                <button wire:click="updateTodo" class="px-4 py-2 bg-blue-500 text-white font-semibold rounded hover:bg-blue-600">Update</button>
                                <button wire:click="cancelEditing" class="px-4 py-2 bg-gray-500 text-white font-semibold rounded hover:bg-gray-600">Cancel</button>
                            @else
                                ...
                                <div class="flex items-center space-x-2">
                                    ...
                                    <button wire:click="deleteTodo({{ $todo->id }})" class="text-sm text-red-500 font-semibold rounded hover:text-red-800 mr-1">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
